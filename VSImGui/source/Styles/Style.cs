...
public class Style
{
    ...
    {
        if (!ComparisonUtils.CompareFloats(mPrevStyle.IndentSpacing, mNativeStyle.IndentSpacing)) result.AppendLine($"style.SpacingIndent = {SpacingIndent}f;");
        if (!ComparisonUtils.CompareFloats(mPrevStyle.ColumnsMinSpacing, mNativeStyle.ColumnsMinSpacing)) result.AppendLine($"style.SpacingColumnsMin = {SpacingColumnsMin}f;");
        if (!ComparisonUtils.CompareFloats(mPrevStyle.WindowBorderSize, mNativeStyle.WindowBorderSize)) result.AppendLine($"style.BorderWindow = {BorderWindow}f;");
        if (!ComparisonUtils.CompareFloats(mPrevStyle.ChildBorderSize, mNativeStyle.ChildBorderSize)) result.AppendLine($"style.BorderChild = {BorderChild}f;");
        if (!ComparisonUtils.CompareFloats(mPrevStyle.PopupBorderSize, mNativeStyle.PopupBorderSize)) result.AppendLine($"style.BorderPopup = {BorderPopup}f;");
        if (!ComparisonUtils.CompareFloats(mPrevStyle.FrameBorderSize, mNativeStyle.FrameBorderSize)) result.AppendLine($"style.BorderFrame = {BorderFrame}f;");
        if (!ComparisonUtils.CompareFloats(mPrevStyle.TabBorderSize, mNativeStyle.TabBorderSize)) result.AppendLine($"style.BorderTab = {BorderTab}f;");
        if (!ComparisonUtils.CompareFloats(mPrevStyle.SeparatorTextBorderSize, mNativeStyle.SeparatorTextBorderSize)) result.AppendLine($"style.BorderSeparatorText = {BorderSeparatorText}f;");
        if (!ComparisonUtils.CompareFloats(mPrevStyle.WindowRounding, mNativeStyle.WindowRounding)) result.AppendLine($"style.RoundingWindow = {RoundingWindow}f;");
        if (!ComparisonUtils.CompareFloats(mPrevStyle.ChildRounding, mNativeStyle.ChildRounding)) result.AppendLine($"style.RoundingChild = {RoundingChild}f;");
        if (!ComparisonUtils.CompareFloats(mPrevStyle.PopupRounding, mNativeStyle.PopupRounding)) result.AppendLine($"style.RoundingPopup = {RoundingPopup}f;");
        if (!ComparisonUtils.CompareFloats(mPrevStyle.FrameRounding, mNativeStyle.FrameRounding)) result.AppendLine($"style.RoundingFrame = {RoundingFrame}f;");
        if (!ComparisonUtils.CompareFloats(mPrevStyle.ScrollbarRounding, mNativeStyle.ScrollbarRounding)) result.AppendLine($"style.RoundingScrollbar = {RoundingScrollbar}f;");
        if (!ComparisonUtils.CompareFloats(mPrevStyle.GrabRounding, mNativeStyle.GrabRounding)) result.AppendLine($"style.RoundingGrab = {RoundingGrab}f;");
        if (!ComparisonUtils.CompareFloats(mPrevStyle.TabRounding, mNativeStyle.TabRounding)) result.AppendLine($"style.RoundingTab = {RoundingTab}f;");
        if (!ComparisonUtils.CompareFloats(mPrevStyle.ScrollbarSize, mNativeStyle.ScrollbarSize)) result.AppendLine($"style.SizeScrollbar = {SizeScrollbar}f;");
        if (!ComparisonUtils.CompareFloats(mPrevStyle.GrabMinSize, mNativeStyle.GrabMinSize)) result.AppendLine($"style.SizeGrabMin = {SizeGrabMin}f;");
        if (!ComparisonUtils.CompareFloats(mPrevStyle.Alpha, mNativeStyle.Alpha)) result.AppendLine($"style.Alpha = {Alpha}f;");
        if (!ComparisonUtils.CompareFloats(mPrevStyle.DisabledAlpha, mNativeStyle.DisabledAlpha)) result.AppendLine($"style.DisabledAlpha = {DisabledAlpha}f;");
        if (!ComparisonUtils.CompareFloats(mPrevStyle.CurveTessellationTol, mNativeStyle.CurveTessellationTol)) result.AppendLine($"style.CurveTessellationTol = {CurveTessellationTol}f;");
        if (!ComparisonUtils.CompareFloats(mPrevStyle.CircleTessellationMaxError, mNativeStyle.CircleTessellationMaxError)) result.AppendLine($"style.CircleTessellationMaxError = {CircleTessellationMaxError}f;");
        if (!ComparisonUtils.CompareFloats(mPrevStyle.HoverStationaryDelay, mNativeStyle.HoverStationaryDelay)) result.AppendLine($"style.HoverStationaryDelay = {HoverStationaryDelay}f;");
        if (!ComparisonUtils.CompareFloats(mPrevStyle.HoverDelayShort, mNativeStyle.HoverDelayShort)) result.AppendLine($"style.HoverDelayShort = {HoverDelayShort}f;");
        if (!ComparisonUtils.CompareFloats(mPrevStyle.HoverDelayNormal, mNativeStyle.HoverDelayNormal)) result.AppendLine($"style.HoverDelayNormal = {HoverDelayNormal}f;");
        if (!ComparisonUtils.CompareFloats(mPrevStyle.LogSliderDeadzone, mNativeStyle.LogSliderDeadzone)) result.AppendLine($"style.LogSliderDeadzone = {LogSliderDeadzone}f;");
        if (!ComparisonUtils.CompareFloats(mPrevStyle.TabMinWidthForCloseButton, mNativeStyle.TabMinWidthForCloseButton)) result.AppendLine($"style.TabMinWidthForCloseButton = {TabMinWidthForCloseButton}f;");
        if (!ComparisonUtils.CompareFloats(mPrevStyle.MouseCursorScale, mNativeStyle.MouseCursorScale)) result.AppendLine($"style.MouseCursorScale = {MouseCursorScale}f;");
    }
